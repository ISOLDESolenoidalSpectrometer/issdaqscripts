...
def file_names(inputfile_directory, inputfile_number):
    ...
    for i in glob.glob(os.path.join(folder, filename_search)):
        filename_in_list += [i]
    filename_out = 'R' + inputfile_number + '_sum_t1' + '.dat'
...
def extract_all_data(name):
    ...
    for i in data:
        if time_str == "":
            time_str = extract_str(i, "T1 events", "=", "")
    print(name, "T1 events: ", time_str)
    return time_str
...
def main(filename_list, filename_out):
    ...
    for i in range(len(filename_list)):
        ...
        timeTotal += time
    print("Total T1 events: ", timeTotal)
    output_file = open(filename_out, 'w')
    output_file.write('T1 events: ' + str(timeTotal))
    output_file.close()
