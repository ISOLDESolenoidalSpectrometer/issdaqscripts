...
void Get110SnStats( std::string file_name, std::string subrun, double time ){
  ...
  int elum_sector_bgd[NUM_SECTORS];
  ...
  for ( int i = 0; i < NUM_SECTORS; ++i ){
    ...
    //    std::cout<<std::stoi(subrun.substr(1,subrun.length()-3))<<std::endl;
    if(std::stoi(subrun.substr(1,subrun.length()-3)) < 31){
      elum_sector_peak[i] = h1->Integral( h1->FindBin( 1500 ), h1->FindBin( 2200 ) );
      elum_sector_bgd[i] = h1->Integral( h1->FindBin( 2500 ), h1->FindBin( 8500 ) );
    }
    else{
      elum_sector_peak[i] = h1->Integral( h1->FindBin( 2500 ), h1->FindBin( 3800 ) );
      elum_sector_bgd[i] = h1->Integral( h1->FindBin( 4000 ), h1->FindBin( 11500 ) );
    }
  ...
  out_file <<
    ...
    elum_sector_bgd[0]/time << "\t" <<
    elum_sector_bgd[1]/time << "\t" <<
    elum_sector_bgd[2]/time << "\t" <<
    elum_sector_bgd[3]/time << "\t" <<
