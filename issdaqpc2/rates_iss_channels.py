...
# Update again to make a channel-by-channel printing for debugging 04/07/2022
import sys
import requests
...
# Check we have module number
if len(sys.argv) == 1:
	print("Give module number as argument: rates_iss.py <mod>")
	quit()

# Settings
MODULE = sys.argv[1]
THRESHOLD = 10000 # change font colour when threshold reached
NEGATIVE = 500000000000 # when the histograms are zeroed, we get a negative value that is unsigned, so a large positive false rate
...
Array1_chs = [6283,6284,6285,6286,6287,6288,6289,6290,6291,6292,6293,6300,6301,6302,6303,6304,6305,6306,6307,6308,6309,6310,6361,6362,6363,6364,6365,6366,6367,6368,6369,6370,6371,6378,6379,6380,6381,6382,6383,6384,6385,6386,6387,6388,6399]
...
Array0_ids = [x-6144 for x in Array0_chs]
Array1_ids = [x-6272 for x in Array1_chs]
Array2_ids = [x-6400 for x in Array2_chs]
Array3_ids = [x-6528 for x in Array3_chs]
Array4_ids = [x-6656 for x in Array4_chs]
Array5_ids = [x-6784 for x in Array5_chs]
Array_chs = [Array0_chs,Array1_chs,Array2_chs,Array3_chs,Array4_chs,Array5_chs]
Array_ids = [Array0_ids,Array1_ids,Array2_ids,Array3_ids,Array4_ids,Array5_ids]
...
def get_rate(bytes, ch):
	rate_ch_hit0 = bytes[ (ch)*4 : (ch)*4+4]
	rate_ch_hit1 = bytes[ (ch-4096)*4 : (ch-4096)*4+4]
	rate  = int.from_bytes(rate_ch_hit0, byteorder='big')
	rate += int.from_bytes(rate_ch_hit1, byteorder='big')
	if rate > NEGATIVE or rate < 0:
		return 0
	else:
		return rate
			...
			if result != '' and result != None:
				...
				for i in range( len(Array_chs) ):
					...
					for j in range( len(Array_chs[i]) ):
						...
						payload += 'detector,name=Array,module=' + str(MODULE) + ',asic=' + str(i) + ',channel=' + str(Array_ids[i][j]) + ' value=' + str(ch_rate) + '\n'
						#print_rates(result, '\t\tch ' + str(j) + ' = ', ch_rate )
						asic_rate += ch_rate
