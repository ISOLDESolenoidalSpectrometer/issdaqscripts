...
def get_rate(bytes, ch):
	rate_ch = bytes[ (ch)*4 : (ch)*4+4]
	return int.from_bytes(rate_ch, byteorder='little')

# Send rates to Influx database
def send_to_influx(name, number, channel, rate ):
	try:
		payload = 'detector,name=' + str(name) + ',number=' + str(number) + ',channel=' + str(channel) + ' value=' + str(rate)
...
if __name__ == '__main__':
	...
	if result != '' and result != None:
		for i in range(len(RecoilE_chs)):
			...
			send_to_influx("Recoil", i+1, "E", get_rate(result,RecoilE_chs[i]) )

		for i in RecoildE_chs:
			print_rates(result, "RecoildE", RecoildE_chs[i])
			send_to_influx("Recoil", i+1, "dE", get_rate(result,RecoildE_chs[i]) )

		for i in range(len(Stub1_chs)):
			print_rates(result, "STUBL " + Stub_ch_names[i], Stub1_chs[i])
			send_to_influx("STUB", 1, Stub_ch_names[i].strip(), get_rate(result, Stub1_chs[i]) )

		for i in range(len(Stub2_chs)):
			print_rates(result, "STUBT " + Stub_ch_names[i], Stub2_chs[i])
			send_to_influx("STUB", 2, Stub_ch_names[i].strip(), get_rate(result, Stub2_chs[i]) )

		for i in range(len(Stub3_chs)):
			print_rates(result, "STUBB " + Stub_ch_names[i], Stub3_chs[i])
			send_to_influx("STUB", 3, Stub_ch_names[i].strip(), get_rate(result, Stub3_chs[i]) )
					
		for i in range(len(Stub4_chs)):
			print_rates(result, "STUBR " + Stub_ch_names[i], Stub4_chs[i])
			send_to_influx("STUB", 4, Stub_ch_names[i].strip(), get_rate(result, Stub4_chs[i]) )
